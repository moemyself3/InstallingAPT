...
## Purpose
This guide is intended to be a walkthrough for terminal installation of Aperture Photometry Tool. 
This was something that was made to facilitate the process due to issues regarding java especially during
the transition between APT versions 2.8.4-3.0.1 and the changes in macOS versions during that time.

It is recommended to install using the dmg from the [APT downloads page](https://aperturephotometry.org/downloads/) directly.

## This Guide is unofficial ✨
As such these instructions and links may stop working at some point. 
If so please open an issue to see if we can update! Use the latest instructions from APT.
### The official guide: https://aperturephotometry.org/installation-execution/

# Video Guides and Install Instructions
...
* For version 3.0.8: http://web.ipac.caltech.edu/staff/laher/apt/APT_v3.0.8.tar.gz
* For version 3.0.2: No longer available, please use most recent version.
...
>If using version 2.8.4 replace `APT_v3.0.8.tar.gz` with `APT_v2.8.4.tar.gz` in the instructions throughout this guide.

Open Finder and double click `APT_v3.0.8.tar.gz`. 
Once you can confirm you have the normal folder named `APT_v3.0.8` delete the file named `APT_v3.0.8.tar.gz`
...
Copy and Paste this into Terminal:
    ...
    echo "export APT_HOME=~/Downloads/APT_v3.0.8
...
## 1. Delete Files
To uninstall APT you would just need to delete the folder APT_v3.0.8.
...
```
export APT_HOME=~/Downloads/APT_v3.0.8
export PATH=$PATH:$APT_HOME
export APT_ARCH=MAC
```
...
```
sed -i '' '/export APT_HOME=~\/Downloads\/APT_v3.0.8/d' .zprofile
```
